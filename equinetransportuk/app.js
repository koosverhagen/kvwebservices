    ...
    vehicles.forEach((vehicle) => {
      ...
      card.innerHTML = `
        <div style="position:relative;">
          ...
          <div class="fleet-img-overlay" onclick="openFleetModal('${vehicle.id}')">
            <p class="fleet-overlay-text">Explore Details</p>
            <button class="apple-btn">More Images</button>
          </div>
        </div>
        <div class="fleet-content">
          <h3>${vehicle.name}</h3>
          <p class="muted tiny">${vehicle.type} · ${vehicle.horses} Horses</p>
          <p><strong>From £${vehicle.dayRate}</strong> / day</p>
          <button class="btn full fleet-card-book" style="margin-top:10px" data-lorry-id="${vehicle.id}">Book this Lorry</button>
        </div>
      `;
